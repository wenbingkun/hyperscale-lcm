...
2.  [ ] **WebSocket 认证补齐** — `DashboardWebSocket` 的 `@OnOpen` 添加 JWT Token 验证（从 query param 或 Sec-WebSocket-Protocol 解析），未认证连接立即关闭；新增 `DashboardWebSocketAuthTest` 回归测试
...
6.  [ ] **JaCoCo 基线上调** — `jacocoMinimumCoverage` 从 `0.30` 上调至 `0.45`，与当前实测覆盖率对齐，防止回退
...
*   **子系统概况**:
    *   Core (Java/Quarkus): JaCoCo 实测覆盖率 ~45%，20 个 Service 中 14 个已有测试，12 个 API Resource 中 6 个已有测试。仍有 6 个 Service 和 4 个 Resource 零测试覆盖。
...
*   **关键结论**:
    *   [x] 前端零测试覆盖风险已开始收敛。
    *   [x] Core 覆盖率基线与 CI 门禁已落地，当前默认基线为 `30%`，实测 ~45%。
    ...
    *   [ ] 安全缺口待收敛：Grafana 默认凭据、WebSocket 无认证、API 无速率限制。
