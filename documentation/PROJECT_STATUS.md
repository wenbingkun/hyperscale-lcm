...
| 集成测试（Testcontainers / DevServices / E2EIntegrationTest） | 🟡 主链路覆盖 | Playwright E2E 已覆盖登录、Dashboard、发现、作业、拓扑、卫星、凭据主流程；断连恢复和压力级故障场景仍不充分 |
...
| Playwright 浏览器级回归 | ✅ 已落地 | 已建立基于 API mock 的 Chromium 回归套件，覆盖登录、Dashboard、发现、作业、拓扑、卫星详情、凭据配置等 7 类主流程，并接入 CI `frontend-e2e` job |
...
| 🟡 P1 | Demo smoke 扩展（与真实后端链路的端到端冒烟） | 回归保护 | 在已有 Playwright mock-browser 回归基础上补真实服务闭环 |
...
- **中期 3-4 周：** Demo smoke、PXE 生产硬化、负载基线
- **远期 5-8 周：** 多集群联邦增强
